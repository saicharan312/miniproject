...
{
		$sql ="UPDATE `fooddepartmentofficer` SET `officername`='".$_POST['officername']."', `cityid`='".$_POST['cityid']."', `stateid`='".$_POST['stateid']."',`officercode`='".$_POST['officercode']."' WHERE officerid='$_SESSION[officerid]'";
		$qsql = mysqli_query($con,$sql);
		if(mysqli_affected_rows($con) ==1)
		{
			echo "<script>alert('Food department Officer Profile Updated successfully...');</script>";
		}
		else
		{
...
$sqledit = "SELECT * FROM fooddepartmentofficer where officerid='$_SESSION[officerid]'";
	...
	<select name="stateid" id="stateid" class="form-control" onchange="loadcity(this.value)">
		<?php
			...
			while($rstudent = mysqli_fetch_array($qsqlstudent)){
				$selected ='';
				if($rsedit['stateid']== $rstudent['stateid']) 
				{
