...
if(isset($_POST['submit']))
{
	...
	if(isset($_FILES["headmasterimg"]["name"]) && $_FILES["headmasterimg"]["name"] !=''){
		...
		`headmastercode`='".$_POST['headmastercode']."',`password`='".$_POST['password']."',`status`='".$_POST['status']."' $condition WHERE schoolheadmasterid='$_SESSION[schoolheadmasterid]'";
		$qsql = mysqli_query($con,$sql);
		if(mysqli_affected_rows($con) ==1)
		{
			echo "<script>alert('School Head Master Profile Updated successfully...');</script>";
...
{
	$sqledit = "SELECT * FROM schoolheadmaster where schoolheadmasterid='$_SESSION[schoolheadmasterid]'";
...
<form method="post" action="" onsubmit="return confirmvalidation()"  enctype="multipart/form-data">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">School Head Master Profile</h3>
        </div>
        <div class="card-body">
