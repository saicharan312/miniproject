...
if(isset($_POST['submit']))
{
	...
	{
		$sql ="UPDATE `beoadmin` SET `adminname`='".$_POST['adminname']."',`loginid`='".$_POST['loginid']."',`password`='".$_POST['password']."',`status`='".$_POST['status']."' WHERE beoadminid='".$_GET['editid']."'";
	...
	{  
		$sql ="INSERT INTO beoadmin SET `adminname`='".$_POST['adminname']."',`loginid`='".$_POST['loginid']."',`password`='".$_POST['password']."' ,`status`='".$_POST['status']."' ";
		
		$qsql = mysqli_query($con,$sql);
		if(mysqli_affected_rows($con) ==1)
		{
			echo "<script>alert('Admin record inserted successfully...');</script>";
			echo "<script>window.location='beoadmin.php';</script>";
		}
		else
		{
			echo mysqli_error($con);
...
if(isset($_GET['editid']))
{
	$sqledit = "SELECT * FROM beoadmin where beoadminid='".$_GET['editid']."'";
...
<form method="post" action="" onsubmit="return confirmvalidation()">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Beo Admin Master</h3>
        </div>
        <div class="card-body">
